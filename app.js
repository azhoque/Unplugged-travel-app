...
function getWeatherApi(searchTerm, callback){
  ...
  function getMapApi(searchTerm, callback){
    const mapApi = 'https://maps.googleapis.com/maps/api/js?query='+ searchTerm +'&key=AIzaSyDWIyUQOV2Ud4rxM4H-hEC6kph2XjxhV_A';
    const api3Call = {
      url: mapApi,
      data: {
        query: `${searchTerm}`,
   ...
   // var temp = out.weather.main;
    ...
    `<div class="col s4">
       ...
       <div class="card-content">
         <span class="card-title activator grey-text text-darken-4">${city}, ${country}<i class="material-icons right">more_vert</i></span>

       </div>
       <div class="card-reveal">
         <span class="card-title grey-text text-darken-4">${city}, ${country}<i class="material-icons right">close</i></span>
         <p>${description}</p><br>
